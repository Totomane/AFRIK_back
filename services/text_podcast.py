...
class PodcastService:
    ...
    @staticmethod
    def generate_podcast_text(country: str, risks: list[str], year: int, tone: str = "serious", title: str = None) -> str:
        """
        ...
        # If title is provided (new format), use it; otherwise use old format for compatibility
        if title:
            text_filename = os.path.join(TEXT_DIR, f"{title}.txt")
        else:
            # Fallback to organized naming: countryname_year_risks
            country_clean = country.replace(" ", "-")
            risks_str = "_".join([risk.replace(" ", "-") for risk in risks])
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            text_filename = os.path.join(TEXT_DIR, f"{country_clean}_{year}_{risks_str}_{timestamp}.txt")
